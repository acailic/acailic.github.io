---
title: Angular
layout: post
tags: [javascript, angular]
date: 2019-09-27
---
## Angular
### Basic Concepts
 - [Everything you need to know about change detection in Angular](https://blog.angularindepth.com/everything-you-need-to-know-about-change-detection-in-angular-8006c51d206f)
 - [Angular 8 for beginners](https://www.techiediaries.com/angular-components/)
 - [Bootstrapping Angular](https://blog.cloudboost.io/bootstrapping-angular-4-app-711722a4e816)
 - [Workspace and project file structure](https://angular.io/guide/file-structure)
 - [Model-View-Presenter with Angular](https://blog.angularindepth.com/model-view-presenter-with-angular-3a4dbffe49bb)
 - [I switched a map and you'll never guess what happened next ](https://youtu.be/rUZ9CjcaCEw)
 - [Zones](https://youtu.be/3IqtmUscE_U)
 - [Angular Change Detection: Default vs. OnPush](https://medium.com/@mikyung.lee11/angular-change-detection-default-vs-onpush-606daf9ea999)
 
### Forms

- [Angular Forms Guide - Template Driven and Reactive Forms](https://blog.angular-university.io/introduction-to-angular-2-forms-template-driven-vs-model-driven/)
- [Custom Form Validators](https://codecraft.tv/courses/angular/advanced-topics/basic-custom-validators/)

### Hooks

- [Lifecycle Hooks](https://angular.io/guide/lifecycle-hooks)
- [Lifecycle Hooks](https://codecraft.tv/courses/angular/components/lifecycle-hooks/)

### Bootstrapping Angular

- [Bootstrapping Angular](https://blog.cloudboost.io/bootstrapping-angular-4-app-711722a4e816)
- [Learn NgModule in Angular with Examples](https://youtu.be/oqZ4-ULwfbc)
- [Angular 7|8 Components](https://www.techiediaries.com/angular-components/)
- [Introduction to services and dependency injection](https://angular.io/guide/architecture-services)
- [A Practical Guide to Angular Directives](https://www.sitepoint.com/practical-guide-angular-directives/)
- [Pipes](https://angular.io/guide/pipes)
- [The essential difference between pure and impure pipes in Angular and why that matters](https://blog.angularindepth.com/the-essential-difference-between-pure-and-impure-pipes-and-why-that-matters-999818aa068)

### Angular Routing

- [A Complete Guide To Routing In Angular](https://www.smashingmagazine.com/2018/11/a-complete-guide-to-routing-in-angular/)
- [Angular 8 Router Tutorial: Learn Routing & Navigation by Example](https://www.techiediaries.com/angular-router/)

### Client-server communication (HttpClient)

- [Angular HTTP Client - Quickstart Guide](https://blog.angular-university.io/angular-http/)
- [JSONP Example with Observables](https://codecraft.tv/courses/angular/http/jsonp-with-observables/)

### Advanced
 - [Dependency injection basics (video)](https://www.youtube.com/watch?v=0X1Ns2NRfks)
 - [Optimize and debug change detection like a pro](https://youtu.be/DsBy9O0c6eo)
 - [https://youtu.be/DsBy9O0c6eo](https://youtu.be/F6rF16TmB0E)
 - [A lot of useful articles about Angular and its ecosystem](https://indepth.dev/)
 - [Authentication in Angular with NgRx Part 1](http://www.gistia.com/authentication-in-angular-with-ngrx-part-i/)
 - [Authentication in Angular with NgRx Part 2](http://www.gistia.com/authentication-in-angular-with-ngrx-part-ii/)
  
